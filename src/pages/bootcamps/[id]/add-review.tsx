...
const EditReviews: NextPage<Props> = ({ bootcampId }) => {
  const {
    register,
    handleSubmit,
    onSubmit,
        ...
        <div className="col-md-8 m-auto">
          <div className="card bg-white py-2 px-4">
            <div className="card-body">
              <Link
                href={`/bootcamps/${bootcampId}`}
                className="btn btn-link text-secondary my-3"
              >
                <FontAwesomeIcon icon={faChevronLeft} /> Bootcamp Info
              </Link>
              <h1 className="mb-2">{data?.data.name}</h1>
              ...
              <form onSubmit={handleSubmit(onSubmit)}>
                <div className="form-group">
                  <label htmlFor="rating">
                    Rating: <span className="text-primary">{watchRating}</span>
                  </label>
                  <input
                    type="range"
